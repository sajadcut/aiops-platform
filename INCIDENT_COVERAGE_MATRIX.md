...
## Evidence currently backing partial scenarios

- `INC-002`: `tests/scenarios/test_postgresql_connection_exhaustion_acceptance.py` proves grounded Database/Application analysis, cross-Agent consensus and reclassification of unsafe restart recommendations; `tests/scenarios/test_database_asset_routing_acceptance.py` proves deterministic Database + Application + Dependency + Infrastructure routing.
- `INC-003`: `tests/scenarios/test_vm_asset_routing_acceptance.py` proves deterministic VM + Infrastructure + Network first-wave routing for a service-down/reachability incident.
- `INC-007`: `tests/scenarios/test_disk_inode_exhaustion_acceptance.py` proves Storage RCA remains grounded in live disk/inode/log Evidence and that a model-proposed `Delete old log files from /var/log` action is reclassified as a high-risk, approval-required, non-executable recommendation.

These tests establish repository-level diagnostic/routing or safety coverage only. They do **not** prove a production write path, staging connector behavior or recovery verification, so the scenarios remain `PARTIAL`.
