...
These tests establish repository-level diagnostic/routing coverage only. They do **not** prove a production write path, staging connector behavior or recovery verification, so the scenarios remain `PARTIAL`.
