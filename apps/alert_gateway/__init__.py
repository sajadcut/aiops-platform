...
class ContextBuilder:
    ...
    async def build_context(self, incident_data: IncidentCreate) -> Dict[str, Any]:
        ...
        # دریافت داده‌ها با gather و return_exceptions=True
    ...
    def _get_mock_logs(self, service: str, now: datetime) -> List[LogEntry]:
        return [
            LogEntry(
                ...
                service=service,
                level="error",
                message="HTTP 500 error on /api/payment: timeout connecting to database",
                source="elasticsearch",
            ...
            LogEntry(
                ...
                service=service,
                level="error",
                message="Failed to fetch user profile: connection refused",
                source="elasticsearch",
