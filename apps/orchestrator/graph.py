...
class WorkflowOrchestrator:
    ...
    async def _parallel_agents_node(self, state: AgentState) -> AgentState:
        ...
        for result in results:
            if isinstance(result, Exception):
                ...
                findings.append(output.model_dump())
    ...
    def _collect_evidence(state: AgentState) -> Dict[str, Any]:
        ...
        for item in evidence:
            ...
            evidence_id = item.get("reference") or item.get("id")
            if evidence_id:
            ...
            message = str(message)
            evidence_text.append(f"[{evidence_type}] {message}")
            ...
            if any(term in normalized for term in [
                ...
                database_signals.append(message)

            if any(term in normalized for term in ["deployment", "deployed", "version"]):
                deployment_signals.append(message)

            if any(term in normalized for term in ["http 500", "500 error", "error_rate", "error rate"]):
                http_error_signals.append(message)
    ...
    def _build_evidence_based_rca(self, state: AgentState) -> Optional[Dict[str, Any]]:
        context = state.get("context", {})
        incident = context.get("incident", {})
        summary = context.get("summary", {})

        collected = self._collect_evidence(state)
        evidence_text = " ".join(collected["evidence_text"]).lower()
        incident_summary = str(incident.get("summary", "")).lower()

        database_signals = collected["database_signals"]
        deployment_signals = collected["deployment_signals"]
        http_error_signals = collected["http_error_signals"]

        has_http_error = bool(http_error_signals)
        has_database_signal = bool(database_signals)
        has_deployment_signal = bool(deployment_signals)

        error_rate = summary.get("error_rate")
        avg_cpu = summary.get("avg_cpu")
        avg_memory = summary.get("avg_memory")

        # Strong database RCA
        if has_http_error and has_database_signal:
            root_cause = (
                "Database connectivity or connection-pool saturation is "
                "the most likely root cause of the HTTP 500 error spike."
            )

            evidence_refs = []
            if http_error_signals:
                evidence_refs.append("HTTP 500/error-rate evidence")
            if database_signals:
                evidence_refs.append("Database connectivity evidence")
            if has_deployment_signal:
                evidence_refs.append("Recent deployment evidence")

            recommendations = [
                "Inspect database connection-pool saturation and active connections",
                "Review database latency and connection timeout errors",
            ]
            ...
            if error_rate is not None and float(error_rate) > 10:
                recommendations.append(
                    f"Investigate the elevated error rate ({float(error_rate):.1f}%)"
                )

            confidence = 0.88
            statement = (
                "Evidence indicates that database connectivity or connection-pool "
                "saturation is the most likely root cause of the HTTP 500 spike."
            )
        ...
        if has_http_error and has_deployment_signal:
            root_cause = (
                "The incident is strongly correlated with a recent application deployment. "
                "The available evidence is insufficient to prove that the deployment is the "
                "sole root cause."
            )
            ...
            return {
                ...
                "evidence_ids": collected["evidence_ids"],
                "recommendations": [
                    "Compare current application behavior with the previous version",
                    "Review deployment logs and application exceptions",
                ],
                "requires_approval": True,
                "root_cause": root_cause,
                "evidence": [
        ...
        if has_http_error:
            return {
                ...
                "finding_type": "root_cause",
                "statement": (
                    "Application-level HTTP errors are confirmed, but the available "
                    "evidence is insufficient to establish a single root cause."
                ),
                "confidence": 0.60,
                "evidence_ids": collected["evidence_ids"],
                "recommendations": [
                ...
                "requires_approval": True,
                "root_cause": "Unknown application failure. Additional evidence is required.",
                "evidence": ["HTTP 500/error-rate evidence"],
                "metrics": {
                    "error_rate": error_rate,
                    "avg_cpu": avg_cpu,
                    "avg_memory": avg_memory,
    ...
    async def _synthesize_node(self, state: AgentState) -> AgentState:
        logger.info("Synthesize node: Building evidence-based RCA")

        state["current_node"] = "synthesize"

        rca = self._build_evidence_based_rca(state)

        if rca:
            state["findings"] = state.get("findings", []) + [rca]

            root_cause = rca.get("root_cause", rca.get("statement", "Unknown"))
            recommendations = rca.get("recommendations", [])
            evidence = rca.get("evidence", [])
            metrics = rca.get("metrics", {})
            ...
            state["final_plan"] = (
                ...
                "EXECUTION POLICY\n"
                "No action has been executed. Human approval is required before "
                "any operational change."
            )

            state["confidence"] = float(rca["confidence"])
            state["messages"] = state.get("messages", []) + ["Evidence-based RCA completed"]

            logger.info(f"Evidence-based RCA completed with confidence={rca['confidence']}")
            return state
...
"""
        ...
        try:
            ...
            state["messages"] = state.get("messages", []) + ["LLM RCA synthesis completed"]

        except Exception as exc:
            logger.error(f"Synthesis failed: {str(exc)}")
            state["final_plan"] = "RCA synthesis failed. Manual review required."
    ...
    async def run(self, initial_state: Dict[str, Any]) -> Dict[str, Any]:
        result = await self.graph.ainvoke(cast(AgentState, initial_state))
        return result