...
async def analyze_incident(request: Request, incident_data: IncidentCreate):
    try:
        ...
        evidence_summary = (
            ...
            f"Log count: {context_summary.get('log_count', 0)}\n"
            f"Metric count: {context_summary.get('metric_count', 0)}\n"
            f"Alert count: {context_summary.get('alert_count', 0)}\n"
            f"Average CPU: {context_summary.get('avg_cpu', 'N/A')}\n"
            f"Average Memory: {context_summary.get('avg_memory', 'N/A')}"
        )

        async with AsyncSessionLocal() as db:
            orchestrator = E2EOrchestrator(db=db)
