...
PROPAGATION_WINDOW_SECONDS = 15 * 60
SHARED_WINDOW_SECONDS = 10 * 60
SENSITIVE_KEYS = {
...
def _rate(value: Optional[float]) -> Optional[float]:
    if value is None:
        return None
    if 1.0 < value <= 100.0:
        return value / 100.0
...
def _flatten_text(value: Any, depth: int = 0) -> List[str]:
    ...
    if isinstance(value, Mapping):
        ...
        for key, current in list(value.items())[:50]:
            ...
            if any(token in normalized for token in SENSITIVE_KEYS):
                continue
            result.extend(_flatten_text(current, depth + 1))
        return result
    if isinstance(value, (list, tuple)):
        result: List[str] = []
        for current in list(value)[:20]:
            result.extend(_flatten_text(current, depth + 1))
        return result
...
def _service(value: Any) -> Optional[str]:
    if value in (None, ""):
        return None
...
def _virtual(role: str, evidence_id: str) -> str:
    safe = re.sub(r"[^a-zA-Z0-9_.:-]+", "-", evidence_id)[:90] or "unknown"
    return f"unknown::{role}::{safe}"


def _service_name(item: Mapping[str, Any]) -> Optional[str]:
    return _service(_field(item, ("service", "service_name", "service.name", "current_service", "app", "application")))


def _peer_name(item: Mapping[str, Any]) -> Optional[str]:
    return _service(_field(item, ("peer_service", "peer.service", "remote_service", "destination_service", "dependency", "dependency_name")))


def _dependency_kind(name: str, hint: Any = None) -> str:
...
def _metric_kind(name: str) -> Optional[str]:
    if any(token in name for token in ("request_rate", "requests_per_second", "requests_total", "rps", "qps", "throughput")):
        return "request_rate"
    if any(token in name for token in ("error_rate", "failure_rate", "errors_per_second", "5xx_rate", "http_5xx")):
        return "error_rate"
    if any(token in name for token in ("latency", "duration", "response_time", "request_time")):
        return "latency"
    if any(token in name for token in ("timeout", "timed_out")):
        return "timeout"
    if any(token in name for token in ("retry", "retries")):
        return "retry"
    if any(token in name for token in ("circuit_breaker", "circuitbreaker", "circuit_open")):
...
def _dependency_semantic(item: Mapping[str, Any]) -> bool:
    ...
    if item_type in TRACE_TYPES | TOPOLOGY_TYPES:
        return True
    explicit_keys = (
    ...
    raw = _raw(item)
    labels = _labels(item)
    if any(key in item or key in raw or key in labels for key in explicit_keys):
        return True
    text = _text(item)
...
def _direct_endpoints(item: Mapping[str, Any], evidence_id: str) -> Tuple[Optional[str], Optional[str]]:
    caller = _service(_field(item, ("caller", "source_service", "client_service", "upstream_service", "from_service", "from")))
    callee = _service(_field(item, ("callee", "target_service", "server_service", "downstream_service", "to_service", "to")))
    current = _service_name(item)
    peer = _peer_name(item)
    item_type = str(item.get("type") or "").lower()
    span_kind = str(_field(item, ("span_kind", "span.kind", "kind")) or "").lower()
    if item_type in TRACE_TYPES and span_kind in {"server", "consumer"}:
        caller = caller or peer
        callee = callee or current
    else:
        caller = caller or current
        callee = callee or peer
    if _dependency_semantic(item) and caller and not callee:
        callee = _virtual("callee", evidence_id)
    elif _dependency_semantic(item) and callee and not caller:
        caller = _virtual("caller", evidence_id)
...
def _observation(item: Mapping[str, Any], index: int, caller: str, callee: str, derived: bool = False) -> Dict[str, Any]:
    ...
    text = _text(item)
    metric_kind = _metric_kind(_metric_name(item)) if item_type == "metric" else None
    ...
    latency = _numeric(_field(item, ("duration_ms", "latency_ms", "elapsed_ms", "duration", "latency")))
    timeout = _numeric(_field(item, ("timeout_rate", "timeouts", "timeout_count")))
    retry = _numeric(_field(item, ("retry", "retries", "retry_count", "retry_rate")))
    if metric_kind == "request_rate":
        ...
        timeout = metric_value
    elif metric_kind == "retry":
        retry = metric_value

    status = str(_field(item, ("status", "span_status", "health", "state")) or "").lower()
    error = bool(_field(item, ("error", "failed", "is_error"))) or status in {"error", "failed", "failure", "unhealthy", "down", "degraded"}
    ...
    circuit_open = bool(_field(item, ("circuit_open", "circuit_breaker_open"))) or any(token in text for token in ("circuit breaker open", "circuit open", "breaker tripped"))

    reasons: List[str] = []
    if error:
        reasons.append("error_observed")
    if timeout_flag:
    ...
    if status in {"unhealthy", "down", "degraded", "failed", "failure", "error"}:
        reasons.append("dependency_health_degraded")
    if metric_kind == "error_rate" and metric_value is not None:
        current, previous = _rate(metric_value), _rate(baseline)
        if previous is not None and current is not None and current - previous >= max(0.01, abs(previous) * 0.5):
            reasons.append("error_rate_regression")
        elif previous is None and current is not None and current >= 0.05:
            reasons.append("elevated_error_rate")
    if metric_kind == "latency" and metric_value is not None:
        if baseline is not None and metric_value > max(baseline * 1.5, baseline + 25.0):
            reasons.append("latency_regression")
    if metric_kind == "timeout" and metric_value is not None and metric_value > 0:
        reasons.append("timeout_metric_nonzero")
    if metric_kind == "retry" and metric_value is not None:
        if baseline is not None and metric_value > max(baseline * 2.0, baseline + 1.0):
            ...
            reasons.append("retry_amplification")
    if any(token in text for token in ("connection refused", "connection reset", "service unavailable", "dependency outage", "upstream failure")):
        reasons.append("dependency_failure_signal")
    ...
    return {
        ...
        "anomaly_reasons": _unique(reasons, 12),
        "dependency_type_hint": _field(item, ("dependency_type", "component_type", "peer_type")),
...
def _expand_topology(evidence: List[Mapping[str, Any]]) -> List[Mapping[str, Any]]:
...
def _collect_observations(evidence: List[Mapping[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, int]]:
    expanded = _expand_topology(evidence)
    observations: List[Dict[str, Any]] = []
    trace_endpoint_count = 0
    trace_resolved_endpoint_count = 0
    spans: Dict[str, Dict[str, Mapping[str, Any]]] = defaultdict(dict)
    trace_span_count = 0
    ...
    for index, item in enumerate(expanded):
        ...
        trace_id = _field(item, ("trace_id", "trace.id"))
        span_id = _field(item, ("span_id", "span.id"))
        if (item_type in TRACE_TYPES or (trace_id and span_id)) and trace_id and span_id:
            spans[str(trace_id)][str(span_id)] = item
            trace_span_count += 1
        if not _dependency_semantic(item):
            continue
        eid = _evidence_id(item, index)
        caller, callee = _direct_endpoints(item, eid)
        if not caller or not callee or caller == callee:
            continue
        row = _observation(item, index, caller, callee)
    ...
    for trace_id, trace_spans in spans.items():
        for span_id, item in trace_spans.items():
            parent_id = _field(item, ("parent_span_id", "parent.span.id"))
            if not parent_id:
                continue
            parent_refs += 1
            current_service = _service_name(item)
            parent = trace_spans.get(str(parent_id))
            if parent is None:
                missing_parent_refs += 1
                if current_service:
                    caller = _virtual(f"trace-parent:{trace_id}", str(parent_id))
                    observations.append(_observation(item, sequence, caller, current_service, derived=True))
                    sequence += 1
                    trace_endpoint_count += 1
                continue
            resolved_parent_refs += 1
            parent_service = _service_name(parent)
            if parent_service and current_service and parent_service != current_service:
                observations.append(_observation(item, sequence, parent_service, current_service, derived=True))
                sequence += 1
                trace_endpoint_count += 1
                trace_resolved_endpoint_count += 1
    ...
    return observations, {
        ...
        "trace_endpoint_observation_count": trace_endpoint_count,
        "trace_resolved_endpoint_observation_count": trace_resolved_endpoint_count,
...
def _latest(rows: List[Dict[str, Any]], field: str, normalize_rate: bool = False) -> Optional[float]:
    ...
    value = _numeric(candidates[-1][field])
    if normalize_rate:
        value = _rate(value)
...
def _metric_summary(rows: List[Dict[str, Any]], field: str, normalize_rate: bool = False) -> Dict[str, Optional[float]]:
    ...
    for row in rows:
        ...
        if value is None:
            continue
        normalized = _rate(value) if normalize_rate else value
        if normalized is not None:
            values.append(normalized)
    if not values:
        return {"latest": None, "min": None, "max": None}
    return {
        "latest": _latest(rows, field, normalize_rate),
...
def _aggregate_edges(observations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for (caller, callee), rows in grouped.items():
        ...
        anomalous = [row for row in rows if row.get("anomaly_reasons")]
        anomaly_times = sorted(row["timestamp"] for row in anomalous if row.get("timestamp") is not None)
        trace_rows = [row for row in rows if row.get("source_type") in TRACE_TYPES]
        error_rate = _latest(rows, "error_rate", True)
        timeout = _latest(rows, "timeout", True)
        if error_rate is None and trace_rows:
            error_rate = round(sum(1 for row in trace_rows if row.get("error")) / len(trace_rows), 6)
        if timeout is None and trace_rows:
            timeout = round(sum(1 for row in trace_rows if row.get("timeout_flag")) / len(trace_rows), 6)
        hints = _unique((row.get("dependency_type_hint") for row in rows), 8)
        kind = _dependency_kind(callee, " ".join(hints))
        changes = [
            {
        ...
        edges.append({
            ...
            "caller": caller,
            "callee": callee,
            "request_rate": _latest(rows, "request_rate"),
            "error_rate": error_rate,
            "latency": _latest(rows, "latency"),
            "timeout": timeout,
            "retry": _latest(rows, "retry"),
            "time_window": {"start": _iso(timestamps[0]) if timestamps else None, "end": _iso(timestamps[-1]) if timestamps else None},
            "anomaly_start": _iso(anomaly_times[0]) if anomaly_times else None,
            "anomaly_reasons": _unique((reason for row in rows for reason in row.get("anomaly_reasons", [])), 20),
            ...
            "anomaly_evidence_ids": _unique((row["evidence_id"] for row in anomalous), 16),
            "trace_observation_count": len(trace_rows),
            "trace_error_count": sum(1 for row in trace_rows if row.get("error")),
            "trace_timeout_count": sum(1 for row in trace_rows if row.get("timeout_flag")),
            "metric_summaries": {
                "request_rate": _metric_summary(rows, "request_rate"),
                "error_rate": _metric_summary(rows, "error_rate", True),
                "latency": _metric_summary(rows, "latency"),
                "timeout": _metric_summary(rows, "timeout", True),
                "retry": _metric_summary(rows, "retry"),
            },
        })
    edges.sort(key=lambda row: (row.get("anomaly_start") is None, row.get("anomaly_start") or "9999", row["edge_id"]))
    return edges


def _nodes(evidence: List[Mapping[str, Any]], edges: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for edge in edges:
        caller, callee = ensure(edge["caller"]), ensure(edge["callee"])
        callee["kind"] = edge["dependency_type"]
        for node in (caller, callee):
            node["evidence_ids"] = _unique([*node["evidence_ids"], *edge["evidence_ids"]], 24)
        callee["regions"] = _unique([*callee["regions"], *edge["regions"]], 12)
        callee["versions"] = _unique([*callee["versions"], *edge["versions"]], 12)
        if edge.get("anomaly_start"):
            callee["anomaly_evidence_ids"] = _unique([*callee["anomaly_evidence_ids"], *edge["anomaly_evidence_ids"]], 20)
    ...
    for index, item in enumerate(evidence):
        ...
        node = ensure(name)
        eid = _evidence_id(item, index)
        node["evidence_ids"] = _unique([*node["evidence_ids"], eid], 24)
        node["regions"] = _unique([*node["regions"], _field(item, ("region", "zone", "availability_zone", "cloud.region"))], 12)
        node["versions"] = _unique([*node["versions"], _field(item, ("version", "release", "service_version", "service.version"))], 12)
        status = str(_field(item, ("health", "health_status", "status", "state")) or "").lower()
        log_error = str(item.get("type") or "").lower() == "log" and any(token in _text(item) for token in ("error", "failed", "timeout", "unavailable", "exception"))
        if status in {"unhealthy", "down", "degraded", "failed", "failure", "error"} or log_error:
            node["explicit_health"] = _unique([*node["explicit_health"], status or "error_signal"], 8)
            node["anomaly_evidence_ids"] = _unique([*node["anomaly_evidence_ids"], eid], 20)
            ts = _iso(_timestamp(item))
            if ts and (node["earliest_anomaly"] is None or ts < node["earliest_anomaly"]):
                node["earliest_anomaly"] = ts
        elif status in {"healthy", "ok", "up", "ready", "available"}:
            node["explicit_health"] = _unique([*node["explicit_health"], status], 8)

    result = []
    unhealthy_values = {"unhealthy", "down", "degraded", "failed", "failure", "error", "error_signal"}
    healthy_values = {"healthy", "ok", "up", "ready", "available"}
    for node in state.values():
        values = set(node["explicit_health"])
        if values & unhealthy_values:
            health = "degraded"
        elif node["anomaly_evidence_ids"]:
            health = "suspected_degraded"
        elif values & healthy_values:
            health = "healthy"
        else:
            health = "unknown"
        result.append({**node, "health_status": health})
...
def _fanout(edges: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for edge in edges:
        grouped[edge["caller"]].append(edge)
    result = [
        {
            "service": caller,
            "dependency_count": len({edge["callee"] for edge in rows}),
            "dependencies": _unique((edge["callee"] for edge in rows), 20),
            "anomalous_dependency_count": sum(1 for edge in rows if edge.get("anomaly_start")),
        }
        for caller, rows in grouped.items()
    ]
    result.sort(key=lambda row: (row["dependency_count"], row["anomalous_dependency_count"]), reverse=True)
    return result[:20]


def _shared(edges: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    incoming: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for edge in edges:
        if edge.get("anomaly_start"):
            incoming[edge["callee"]].append(edge)
    result = []
    for callee, rows in incoming.items():
        callers = sorted({edge["caller"] for edge in rows})
        if len(callers) < 2:
            continue
        times = sorted(filter(None, (_parse_ts(edge["anomaly_start"]) for edge in rows)))
        clustered = bool(times and (times[-1] - times[0]).total_seconds() <= SHARED_WINDOW_SECONDS)
        result.append({
            "dependency": callee,
            "dependency_type": rows[0]["dependency_type"],
            ...
            "evidence_ids": _unique((eid for edge in rows for eid in edge["anomaly_evidence_ids"]), 20),
        })
    result.sort(key=lambda row: (row["clustered_in_time"], row["affected_caller_count"]), reverse=True)
    return result[:16]


def _retry(edges: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    result = []
    for edge in edges:
        retry = _numeric(edge.get("retry"))
        request_rate = _numeric(edge.get("request_rate"))
        ratio = retry / abs(request_rate) if retry is not None and request_rate not in (None, 0) else None
        explicit = "retry_amplification" in edge["anomaly_reasons"] or "retry_regression" in edge["anomaly_reasons"]
        if not explicit and not (ratio is not None and ratio >= 0.20):
            continue
        result.append({
            ...
            "retry": retry,
            "request_rate": request_rate,
            "retry_to_request_ratio": round(ratio, 4) if ratio is not None else None,
            "anomaly_start": edge.get("anomaly_start"),
            "evidence_ids": edge["evidence_ids"][:12],
        })
...
def _timeout_propagation(edges: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    timed_out = [
        ...
        if edge.get("anomaly_start") and any(reason in edge["anomaly_reasons"] for reason in ("timeout_observed", "timeout_metric_nonzero"))
    ]
    incoming: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for edge in timed_out:
        incoming[edge["callee"]].append(edge)
    result = []
    for downstream in timed_out:
        start = _parse_ts(downstream["anomaly_start"])
        if not start:
            continue
        for upstream in incoming.get(downstream["caller"], []):
            end = _parse_ts(upstream["anomaly_start"])
            if not end or end < start:
                continue
            delta = (end - start).total_seconds()
            if delta > PROPAGATION_WINDOW_SECONDS:
                continue
            result.append({
                ...
                "evidence_ids": _unique([*downstream["anomaly_evidence_ids"], *upstream["anomaly_evidence_ids"]], 20),
            })
    result.sort(key=lambda row: row["propagation_seconds"])
    return result[:20]


def _cascades(edges: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for root in anomalous:
        ...
        current = root
        while len(chain) < 8:
            current_ts = _parse_ts(current["anomaly_start"])
            candidates: List[Tuple[datetime, Dict[str, Any]]] = []
            for upstream in incoming.get(current["caller"], []):
                if upstream["edge_id"] in seen:
                    continue
                ts = _parse_ts(upstream["anomaly_start"])
                if not ts or not current_ts or ts < current_ts:
                    continue
                if (ts - current_ts).total_seconds() <= PROPAGATION_WINDOW_SECONDS:
                    candidates.append((ts, upstream))
            if not candidates:
                break
        ...
        if len(chain) >= 2:
            result.append({
                ...
                "evidence_ids": _unique((eid for edge in chain for eid in edge["anomaly_evidence_ids"]), 24),
            })
...
def _critical_paths(edges: List[Dict[str, Any]], service_name: Optional[str]) -> List[Dict[str, Any]]:
    ...
    starts.extend(node for node in sorted(nodes) if indegree[node] == 0 and node not in starts)
    if not starts:
        starts = sorted(nodes)[:4]
    paths: List[Dict[str, Any]] = []

    def record(path: List[Dict[str, Any]]) -> None:
        latencies = [float(edge["latency"]) for edge in path if edge.get("latency") is not None]
        bad = [edge for edge in path if edge.get("anomaly_start")]
        score = min(1.0, 0.18 * len(bad) + 0.02 * len(path) + min(sum(latencies) / 5000.0, 0.5))
        paths.append({
            "nodes": [path[0]["caller"], *[edge["callee"] for edge in path]],
            "edges": [edge["edge_id"] for edge in path],
            "cumulative_latency": round(sum(latencies), 3) if latencies else None,
            "anomalous_edges": [edge["edge_id"] for edge in bad],
            "earliest_anomaly": min((edge["anomaly_start"] for edge in bad), default=None),
            "path_score": round(score, 4),
            "policy": "critical_path_candidate_not_root_cause",
        })

    def walk(node: str, path: List[Dict[str, Any]], seen: Set[str]) -> None:
        if len(path) >= 8 or not adjacency.get(node):
            if path:
                record(path)
            return
        progressed = False
        for edge in adjacency[node][:12]:
            if edge["callee"] in seen:
                continue
            progressed = True
            walk(edge["callee"], [*path, edge], {*seen, edge["callee"]})
        if path and not progressed:
            record(path)

    for start in starts[:6]:
        walk(start, [], {start})
    unique = {tuple(path["edges"]): path for path in paths}
    result = list(unique.values())
    result.sort(key=lambda row: (row["path_score"], row.get("cumulative_latency") or 0.0), reverse=True)
    return result[:12]


def _root_contributors(edges: List[Dict[str, Any]], nodes: List[Dict[str, Any]], shared: List[Dict[str, Any]], retry_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    anomalous = [edge for edge in edges if edge.get("anomaly_start")]
    if not anomalous:
        return []
    anomalous.sort(key=lambda edge: edge["anomaly_start"])
    earliest = _parse_ts(anomalous[0]["anomaly_start"])
    node_map = {node["name"]: node for node in nodes}
    shared_map = {row["dependency"]: row for row in shared}
    retry_edges = {row["edge_id"] for row in retry_rows}
    incoming: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    ...
    result = []
    for edge in anomalous:
        ts = _parse_ts(edge["anomaly_start"])
        if not ts or not earliest:
            continue
        delta = max(0.0, (ts - earliest).total_seconds())
        first_factor = 1.0 if delta <= 1.0 else 0.0
        earliest_factor = 1.0 / (1.0 + delta / 300.0)
        downstream_health = str(node_map.get(edge["callee"], {}).get("health_status") or "unknown")
        ...
        retry_factor = 1.0 if edge["edge_id"] in retry_edges else 0.0
        for upstream in incoming.get(edge["caller"], []):
            upstream_ts = _parse_ts(upstream["anomaly_start"])
            if upstream_ts and upstream_ts >= ts and upstream["edge_id"] in retry_edges:
                retry_factor = 1.0
        shared_row = shared_map.get(edge["callee"])
        shared_factor = 1.0 if shared_row and shared_row["clustered_in_time"] else 0.5 if shared_row else 0.0

        later_upstream = []
        for upstream in incoming.get(edge["caller"], []):
            upstream_ts = _parse_ts(upstream["anomaly_start"])
            if upstream_ts and upstream_ts >= ts and (upstream_ts - ts).total_seconds() <= PROPAGATION_WINDOW_SECONDS:
                later_upstream.append(upstream)
        later_shared = []
        for peer in incoming.get(edge["callee"], []):
            if peer["edge_id"] == edge["edge_id"]:
                continue
            peer_ts = _parse_ts(peer["anomaly_start"])
            if peer_ts and peer_ts >= ts and (peer_ts - ts).total_seconds() <= SHARED_WINDOW_SECONDS:
                later_shared.append(peer)
        deeper_earlier = []
        for deeper in outgoing.get(edge["callee"], []):
            deeper_ts = _parse_ts(deeper["anomaly_start"])
            if deeper_ts and deeper_ts < ts:
        ...
        strength = 0.5 if edge["anomaly_reasons"] else 0.0
        error_rate, timeout = _rate(_numeric(edge.get("error_rate"))), _rate(_numeric(edge.get("timeout")))
        if error_rate is not None:
            strength = max(strength, min(1.0, error_rate / 0.25))
        if timeout is not None:
            strength = max(strength, min(1.0, timeout / 0.10))
        if edge["circuit_breaker"] == "open":
            strength = max(strength, 0.9)

        conflicts: List[str] = []
        penalty = 0.0
        if deeper_earlier:
            conflicts.append("deeper_downstream_edge_failed_earlier")
            penalty += 0.25
        if downstream_health == "healthy":
            conflicts.append("callee_has_explicit_healthy_evidence")
            penalty += 0.20
        if edge["caller"].startswith("unknown::") or edge["callee"].startswith("unknown::"):
            conflicts.append("virtual_or_uninstrumented_endpoint")
            penalty += 0.08
        ...
        result.append({
            ...
            "shared_node_effect": {
                ...
                "affected_callers": list(shared_row["affected_callers"]) if shared_row else [],
                "clustered_in_time": bool(shared_row["clustered_in_time"]) if shared_row else False,
            },
            ...
            "evidence_ids": edge["anomaly_evidence_ids"][:16],
            "falsification_checks": [
                f"Verify direct health of {edge['callee']} during the anomaly window",
...
def _trace_quality(stats: Mapping[str, int], observations: List[Dict[str, Any]], evidence: List[Mapping[str, Any]]) -> Dict[str, Any]:
    span_count = int(stats.get("trace_span_count", 0))
    endpoint_count = int(stats.get("trace_endpoint_observation_count", 0))
    resolved_endpoints = int(stats.get("trace_resolved_endpoint_observation_count", 0))
    parent_refs = int(stats.get("parent_reference_count", 0))
    missing_parents = int(stats.get("missing_parent_reference_count", 0))
    trace_present = span_count > 0 or any(str(item.get("type") or "").lower() in TRACE_TYPES for item in evidence)
    topology_present = any(str(item.get("type") or "").lower() in TOPOLOGY_TYPES for item in evidence)
    if trace_present:
        endpoint_ratio = resolved_endpoints / endpoint_count if endpoint_count else (1.0 if span_count and not parent_refs else 0.0)
        parent_ratio = (parent_refs - missing_parents) / parent_refs if parent_refs else 1.0
    ...
    return {
        "mode": mode,
        "trace_evidence_present": trace_present,
...
def _inventory(edges: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    result: Dict[str, List[Dict[str, Any]]] = {
        "external_api": [], "database": [], "messaging": [], "identity": [], "regional": [], "version_or_change": []
    }
    for edge in edges:
        base = {
            "edge_id": edge["edge_id"], "caller": edge["caller"], "callee": edge["callee"],
            "anomaly_start": edge.get("anomaly_start"), "evidence_ids": edge["evidence_ids"][:12],
        }
        kind = edge["dependency_type"]
        if kind in {"external_api", "database", "messaging", "identity"}:
            result[kind].append(base)
        if edge["regions"]:
            result["regional"].append({**base, "regions": edge["regions"]})
        if edge["versions"] or edge["dependency_version_changes"]:
            result["version_or_change"].append({**base, "versions": edge["versions"], "changes": edge["dependency_version_changes"]})
    return {key: rows[:16] for key, rows in result.items()}
...
def build_dependency_causal_analysis(
    ...
    *,
    ...
    observations, trace_stats = _collect_observations(live)
    edges = _aggregate_edges(observations)
    nodes = _nodes(live, edges)
    fanout = _fanout(edges)
    shared = _shared(edges)
    retries = _retry(edges)
    timeout_propagation = _timeout_propagation(edges)
    cascades = _cascades(edges)
    critical_paths = _critical_paths(edges, service_name)
    roots = _root_contributors(edges, nodes, shared, retries)
    trace = _trace_quality(trace_stats, observations, live)
    inventory = _inventory(edges)

    anomalous = sorted((edge for edge in edges if edge.get("anomaly_start")), key=lambda edge: edge["anomaly_start"])
    first = None
    if anomalous:
        edge = anomalous[0]
        first = {
            "edge_id": edge["edge_id"], "caller": edge["caller"], "callee": edge["callee"],
            "timestamp": edge["anomaly_start"], "evidence_ids": edge["anomaly_evidence_ids"][:12],
        }
    timeline = [
        {
            "timestamp": edge["anomaly_start"], "edge_id": edge["edge_id"], "caller": edge["caller"],
            "callee": edge["callee"], "reasons": edge["anomaly_reasons"], "evidence_ids": edge["anomaly_evidence_ids"][:12],
        }
        for edge in anomalous[:40]
    ]
    ...
    handoff_map = {"database": "database", "messaging": "messaging", "identity": "identity", "external_api": "network"}
    handoffs: List[Dict[str, Any]] = []
    for root in roots[:6]:
        target = handoff_map.get(root["dependency_type"])
        if target:
            handoffs.append({
                "agent": target,
                "reason": f"root contributor candidate terminates at {root['dependency_type']} dependency {root['callee']}",
                "evidence_ids": root["evidence_ids"][:12],
            })
    if inventory["version_or_change"]:
        handoffs.append({
            "agent": "change",
            "reason": "dependency version/change metadata overlaps the dependency graph",
            "evidence_ids": _unique((eid for row in inventory["version_or_change"] for eid in row["evidence_ids"]), 12),
        })

    score = float(roots[0]["root_contributor_score"]) if roots else 0.0
    score = min(score, float(trace["confidence_ceiling"]))
    circuits = [
        {
            "edge_id": edge["edge_id"], "caller": edge["caller"], "callee": edge["callee"],
            "state": "open", "anomaly_start": edge.get("anomaly_start"), "evidence_ids": edge["anomaly_evidence_ids"][:12],
        }
        for edge in edges if edge["circuit_breaker"] == "open"
    ][:16]
    ...
    return {
        ...
        "dependency_graph": {
            ...
            "nodes": nodes[:80],
            "edges": edges[:120],
            "unknown_virtual_nodes": [node for node in nodes if node["virtual"]][:24],
        ...
        "first_failing_edge": first,
        "earliest_anomaly": first["timestamp"] if first else None,
