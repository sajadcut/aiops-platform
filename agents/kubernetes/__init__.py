...
class KubernetesAgent(BaseAgent):
    ...
    async def analyze(self, input_data: AgentInput) -> AgentOutput:
        ...
        prompt = f"""You are a Kubernetes SRE. LIVE EVIDENCE is authoritative. Knowledge RAG and Operational Memory are auxiliary only and may suggest known failure patterns or runbook checks, never prove current cluster state. Do not claim pod states, rollout failures, OOMKills, scheduling failures or network faults unless evidenced.
...
Hypotheses entries: hypothesis, probability, evidence_ids, conflicting_evidence_ids, falsification_checks. Only live evidence IDs may be cited. immediate_checks must be read-only kubectl/metrics/log inspection.
