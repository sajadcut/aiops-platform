...
class FakeKubernetesConnector:
    ...
    async def restart_workload(self, target, namespace, approval_id, incident_id):
        self.calls.append(("restart", target, namespace, approval_id, incident_id))
        return {"success": True, "action": "restart"}

    async def rollback_workload(self, target, namespace, approval_id, incident_id, revision=None):
        self.calls.append(("rollback", target, namespace, approval_id, incident_id, revision))
        return {"success": True, "action": "rollback"}

    async def scale_workload(self, target, namespace, replicas, approval_id, incident_id):
        self.calls.append(("scale", target, namespace, replicas, approval_id, incident_id))
...
async def test_kubernetes_write_requires_complete_approval_binding():
    ...
    assert await tool.validate(ToolInput(**base, approval_id="a", incident_id="i", approval_granted=True)) is True


@pytest.mark.asyncio
async def test_kubernetes_scale_is_bounded_and_approval_context_is_forwarded():
    ...
    invalid = ToolInput(
        ...
        approval_id="approval-1", incident_id="incident-1", approval_granted=True,
    )
    ...
    assert connector.calls == [("scale", "deployment/payments", "aiops-oat", 4, "approval-1", "incident-1")]
