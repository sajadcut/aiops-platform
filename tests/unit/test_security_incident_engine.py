...
def test_failed_login_storm_is_observed_not_confirmed_compromise():
    evidence = [
        log(
            ...
            timestamp=f"2026-09-16T10:0{index}:00Z",
            user="alice",
    ...
    result = build_security_analysis(evidence, service_name="payments-api")
...
def test_expired_token_is_false_positive_candidate_not_credential_theft():
    result = build_security_analysis([
...
def test_suspicious_process_and_privilege_signals_build_structured_hypothesis():
    result = build_security_analysis([
        log(
            "proc",
            "unexpected binary process execution execve followed by privilege escalation root shell",
            source="edr",
            host="vm-payments-1",
            user="appuser",
            service="payments-api",
...
def test_network_scan_like_activity_is_aggregated_across_destinations():
    ...
    result = build_security_analysis(evidence)
...
def test_insufficient_evidence_requests_verification_instead_of_attack_label():
    result = build_security_analysis([
        log("ordinary", "application request completed normally", service="payments-api"),
    ])
...
def test_security_prompt_projection_redacts_secret_token_values_in_keys_and_free_text():
    projected = security_prompt_evidence([
        log(
            "secret",
            "service account token=SUPER-SECRET-TOKEN password:hunter2 Bearer abc.def.ghi",
            token="RAW-TOKEN",
            secret="RAW-SECRET",
...
async def test_agent_blocks_llm_compromise_upgrade_and_keeps_containment_approval_required():
    payload = {
        ...
        "authentication_signals": ["failed login"],
        "authorization_signals": [],
        "suspicious_signals": ["confirmed compromise"],
        "exposure_signals": [],
        "policy_signals": [],
        "affected_components": ["payments-api"],
        "hypotheses": [{
            "hypothesis": "confirmed credential theft",
            "probability": 0.99,
            "evidence_ids": ["login"],
            "conflicting_evidence_ids": [],
            "falsification_checks": ["check identity audit"],
            "recommended_next_evidence": ["identity audit"],
            "alternative_benign_explanations": ["expired credential"],
            "required_verification": ["verify successful session from same source"],
            "affected_identities": ["alice"],
            "affected_assets": ["payments-api"],
        }],
        "missing_evidence": [],
        "handoff_agents": [],
        "immediate_checks": ["Inspect identity audit"],
        "containment_recommendations": ["Revoke alice sessions and isolate payments host"],
        "confidence": 0.99,
    }
    result = await SecurityAgent(StaticSecurityLLM(payload)).analyze(AgentInput(
        incident_id="sec-1",
        service_name="payments-api",
        evidence_summary="failed login",
