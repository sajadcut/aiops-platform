...
| 6 | Execution boundary | PASS | Approval-gated writes require a durable approved record, exact execution binding, atomic one-time consumption and trusted Control-Plane propagation of the resulting approval context. |
...
| 9 | VM/Kubernetes write | REAL ENV REQUIRED | Real mutation must succeed only through allowlisted MCP write paths, distinct write identity, durable bound approval context and independent post-execution verification. Arbitrary targets/actions or unapproved writes must fail in staging. |
...
6. No Agent or internal caller can bypass policy/approval; approval-gated writes require durable binding validation, one-time consumption, allowlisted tools and audit correlation.
...
9. VM/Kubernetes real writes are constrained to controlled MCP write paths and verified after execution.
