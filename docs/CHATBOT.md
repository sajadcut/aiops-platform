...
For clear live operational reads, a model-only answer is not accepted as current evidence. An unambiguous read-only VM service status/log request is routed deterministically before the intent LLM is called; the reliability wrapper resolves only safe deterministic context: an explicit service plus an explicit VM target, the most recent unambiguous target, or a partial two-octet shorthand such as `6.200` reconstructed from one recent network prefix. It then emits the same structured read-only tool call the model should have selected. No mutation is ever inferred this way. If deterministic resolution is not possible, one bounded repair attempt remains available; the repaired response must select a governed tool or the request fails closed instead of persisting guessed or stale operational data. Short follow-ups therefore reuse only unambiguous operator-provided context and the resolved target is always re-read through the governed tool boundary. VM target/service allowlists remain authoritative: if the edge MCP returns `vm_target_not_allowed` or `vm_service_not_allowed`, the denial is preserved only as that allowlisted policy code, execution stays blocked, and the operator receives a non-retryable authorization message naming the relevant configuration key. Arbitrary remote MCP error text is never propagated to the UI.
