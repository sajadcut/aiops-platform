...
The Kibana API contract for `POST /api/agent_builder/mcp` supports both API key authentication and HTTP Basic authentication. For API-key/OAuth deployments, configure the complete Authorization value in `ELASTICSEARCH_MCP_AUTH_HEADER`. When an existing Kibana service user is used, configure `ELASTICSEARCH_MCP_USERNAME` and `ELASTICSEARCH_MCP_PASSWORD`; the adapter constructs `Authorization: Basic <base64(username:password)>` at runtime. Configure exactly one authentication path. API keys remain preferable for long-lived unattended machine-to-machine deployments because they can be independently scoped, expired, and revoked, but Basic auth is a supported Kibana MCP API mechanism. The generic/internal `MCP_BEARER_TOKEN` is never forwarded to Kibana. The authenticated identity must have `agentBuilder:read` plus the index privileges required by the allowlisted ES|QL tool. Space scoping should be used where it improves isolation.
