...
- Elastic Agent Builder MCP lacks its explicit provider Authorization header;
...
| `ELASTIC_STACK_VERSION`, `ELASTICSEARCH_MCP_URL`, `ELASTICSEARCH_MCP_PROTOCOL_VERSION`, `ELASTICSEARCH_MCP_AUTH_HEADER`, `ELASTICSEARCH_MCP_USERNAME`, `ELASTICSEARCH_MCP_PASSWORD` | Elastic Agent Builder MCP | auth header/password Yes | Stack must be >=9.2; URL must be the default or Space-aware Agent Builder MCP path; provider protocol baseline is `2024-11-05`. The Kibana MCP API supports API key and HTTP Basic authentication. Configure either a complete `ApiKey`/`Bearer`/`Basic` Authorization value or a username/password pair; the client builds Basic auth at runtime. Do not configure both. The generic MCP bearer is never forwarded. |
...
| `ELASTIC_ANOMALY_ACCEPT_INTERIM`, `ELASTIC_ANOMALY_CONTEXT_LOOKBACK_SECONDS`, `ELASTIC_ANOMALY_CONTEXT_LOOKAHEAD_SECONDS`, `ELASTIC_ANOMALY_JOB_SERVICE_MAP` | Elastic ML anomaly webhook + trigger-time enrichment | No | Interim anomalies are ignored by default; context bounds are typed; job-to-service map is a deterministic fallback when the rule payload cannot identify a service. |
| `PROMETHEUS_MCP_URL`, `PROMETHEUS_MCP_PROTOCOL_VERSION`, `PROMETHEUS_MCP_SERVICE_LABEL`, `PROMETHEUS_MCP_AUTH_HEADER` | official `prometheus/prometheus-mcp` client | auth header Yes | Streamable HTTP `/mcp`; supported baseline pins protocol `2025-11-25`. The upstream server forwards `Authorization` to Prometheus, so the generic `MCP_BEARER_TOKEN` is intentionally not inherited by this adapter; use `PROMETHEUS_MCP_AUTH_HEADER` only for deliberate backend credential passthrough. |
| `PROMETHEUS_ALERT_CONTEXT_LOOKBACK_SECONDS`, `PROMETHEUS_ALERT_CONTEXT_LOOKAHEAD_SECONDS` | Alertmanager-triggered evidence enrichment | No | Bound the read-only cross-source evidence window around each alert's `startsAt` timestamp. |
...
| `SSH_ALLOWED_TARGETS`, `SSH_ALLOWED_SERVICES` | VM MCP authorization | No | Must be non-empty in production VM edge. These are the authoritative target/service allowlists for fixed VM reads; the same service boundary also applies to governed writes, which still additionally require Decision/Approval/Execution controls. There is no separate chatbot `read allowlist`. |
