...
| `ELASTIC_STACK_VERSION`, `ELASTICSEARCH_MCP_URL`, `ELASTICSEARCH_MCP_PROTOCOL_VERSION`, `ELASTICSEARCH_MCP_AUTH_HEADER` | Elastic Agent Builder MCP | auth header Yes | Stack must be >=9.2; URL must be the default or Space-aware Agent Builder MCP path; provider protocol baseline is `2024-11-05`. Production requires an explicit Elastic `ApiKey` or documented OAuth `Bearer` header; the generic MCP bearer is not forwarded. |
