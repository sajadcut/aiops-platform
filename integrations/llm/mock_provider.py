...
class MockLLMProvider(LLMAdapter):
    """Deterministic development/test provider. Construction is forbidden in production."""
    ...
    async def generate(
        ...
        **kwargs,
        ...
        logger.info("MockLLM generating development/test response")
        if "triage" in prompt.lower():
            if "http 500" in prompt.lower() or "error rate" in prompt.lower():
                response_content = json.dumps({
                    "incident_type": "application",
                    "severity": "High",
                    "description": "Application error spike detected, likely deployment-related",
                    "confidence": 0.75,
                    "recommendations": ["Check deployment logs", "Rollback if policy permits"],
                })
            elif "cpu" in prompt.lower() or "memory" in prompt.lower():
                response_content = json.dumps({
                    "incident_type": "infrastructure",
                    "severity": "High",
                    "description": "Resource saturation detected",
                    "confidence": 0.65,
                    "recommendations": ["Inspect resource evidence", "Check node health"],
                })
            else:
                response_content = json.dumps({
                    "incident_type": "application",
                    "severity": "Medium",
                    "description": "General incident; additional evidence is required",
                    "confidence": 0.5,
            ...
            response_content = json.dumps({
                ...
                "confidence": 0.7,
                "immediate_actions": ["Review service health", "Check configuration"],
            })
