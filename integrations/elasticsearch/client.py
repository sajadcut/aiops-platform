...
class ElasticsearchClient(BaseConnector):
    ...
    async def get_logs(self, service: str, since: datetime, until: Optional[datetime] = None, level: Optional[str] = None, limit: int = 100) -> List[LogEntry]:
        service_query = f'(service:"{service}" OR service.name:"{service}" OR labels.service:"{service}" OR kubernetes.labels.app:"{service}")'
        rows = await self.search_logs(service_query + (f' AND (level:"{level}" OR log.level:"{level}")' if level else ""), since.isoformat(), until.isoformat() if until else None, limit)
        result = []
        for row in rows:
