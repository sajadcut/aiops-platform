...
function renderDetail(tab = 'overview') {
  ...
  if (tab === 'memory') {
    ...
    pane.innerHTML = `<div class="insight-card"><h4>Operational Memory write-back</h4><p>Only conclusive verified outcomes may be persisted. Cognia Knowledge remains a separate governed domain.</p><div class="json">${esc(JSON.stringify(writeback, null, 2))}</div></div><div class="insight-card"><h4>Related operational memory</h4>${memory?.error ? `<p>${esc(memory.error)}</p>` : ''}<div class="json">${esc(JSON.stringify(memory?.items || [], null, 2))}</div></div>`;
  }
